...
function Product(props) {
  ...
  return (
    <>
      <FileUpload
        uploadDoc={productImageType.file ? true : false}
        ...
        <Loader />
          ...
          <div className="formInput">
            <label htmlFor="Product Description">Product Description</label>
                  <textarea name="description"  value={product && product.description.join('\n')}  onChange={(e)=>setProduct({...product,description: (e.target.value).split('\n')})}  cols="30" rows="10"></textarea>
          </div>
          ...
          <div className="formInput formInputSingleRow formInputChoiceTick">
            ...
            <div className="buttons">
                    <button onClick={()=>addNewPlusPoint()} className="btn1__primary">Add New</button>
                  </div>
            <ul className="points">
                    {
                      product && product.plusPoints && product.plusPoints.map((point,index)=>(
                        <li contentEditable  onBlur={e=>editPlusPoints(index,e.target.innerText)}   key={index}>{point} <span  onClick={()=>deletePlusPoints(index)}>x</span></li>
                      ))
                    }
            </ul>
          </div>
          <div className="plusPoints">
              ...
              <label htmlFor="PlusPoints">FEATURES</label>
                  ...
                  <div className="buttons">
                    <button onClick={()=>addNewFeature()} className="btn1__primary">Add New</button>
            </div>
            <ul className="points">
                    {
                      product && product.features && product.features.map((point,index)=>(
                        <li contentEditable  onBlur={e=>editFeatures(index,e.target.innerText)}   key={index}>{point}  <span  onClick={()=>deleteFeature(index)}>x</span></li>
                      ))
                    }
            </ul>
                </div>
          <div className="techSpecs">
            ...
            <div className="buttons">
                    <button onClick={()=>addNewSpec()} className="btn1__primary">Add New</button>
                  </div>
            <table>
              {
                product && product.specs && product.specs.map((spec,index)=>(
                  <tr>
                          <td colSpan="1" contentEditable  onBlur={e=>editSpecs(index,e.target.innerText,'title')}>{spec.title}</td>
                          <td colSpan="1" contentEditable  onBlur={e=>editSpecs(index,e.target.innerText,'desc')}>{spec.desc}</td>
                          <td colSpan="1"><button className="btn2__primary" onClick={()=>deleteSpecs(index)}>Delete spec</button></td>
                        </tr>
                      )
                      )
                    }
