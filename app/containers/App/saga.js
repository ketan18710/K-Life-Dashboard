...
function* getDataFunc() {
  try {
    ...
    const { statusCode, message } = userData;
...
function* saveDataFunc(data) {
  ...
  try {
    const userData = yield call(request, `${PROD_DOMAIN}/data/publish`, {
      method: 'POST',
...
function* resetPasswordSagaFunc(data) {
  // debugger
  try {
    const reset = yield call(request, `${PROD_DOMAIN}/user/passwordreset`, {
      method: 'PATCH',
...
function* loginFunc(data) {
  try {
    const login = yield call(request, `${PROD_DOMAIN}/user/login`, {
      method: 'POST',
      body: JSON.stringify(data.payload),
      headers: {
        'Content-type': 'application/json; charset=UTF-8',
...
function* saveImageFunc(data) {
  ...
  // // debugger
  try {
    const image = yield call(request, `${PROD_DOMAIN}/data/v1/upload`, {
      method: 'POST',
      body: data.payload,
      headers: {
